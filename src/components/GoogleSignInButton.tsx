...
function GoogleSignInButton({ disabled = false, onClick }: GoogleSignInButtonProps) {
  return (
    <button
      type="button"
      onClick={onClick}
      disabled={disabled}
      className="relative flex min-h-11 w-full items-center justify-center rounded-md border border-[#747775] bg-white px-12 py-2.5 font-sans text-sm font-medium text-[#1f1f1f] shadow-sm transition hover:border-[#5f6368] hover:bg-[#f8faff] hover:shadow-md active:bg-[#f0f4f9] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#0b57d0] disabled:cursor-not-allowed disabled:opacity-50"
    >
      <svg className="absolute left-4 h-4.5 w-4.5" viewBox="0 0 18 18" aria-hidden="true">
        <path
          fill="#4285F4"
